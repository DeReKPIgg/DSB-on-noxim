...
unsigned int GlobalStats::getTotalFlits()
{
	unsigned int n = 0;
...
void GlobalStats::showStats(std::ostream &out, bool detailed)
{
	...
	// int total_cycles = GlobalParams::simulation_time - GlobalParams::stats_warm_up_time;
	out << "% Total flits: " << getTotalFlits() << endl;
	out << "% Total received packets: " << getReceivedFlits() / FLITS_IN_PACKET << endl;
	out << "% Total received flits: " << getReceivedFlits() << endl;
	// out << "% Received/Ideal flits Ratio: " << getReceivedIdealFlitRatio() << endl;
	out << "% Received/Ideal flits Ratio: " << getReceivedFlits() / (double)getTotalFlits() << endl;
	...
	int total_cycles = GlobalParams::simulation_time - GlobalParams::stats_warm_up_time;
	double penalty_avr_delay = (getAverageDelay() * getReceivedFlits() + total_cycles * (getTotalFlits() - getReceivedFlits())) / getTotalFlits();
	out << "% Global average delay with penalty (cycles): " << penalty_avr_delay << endl;
